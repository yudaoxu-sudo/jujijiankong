...
- `inference`：日期、截图金额舍入区间、代币和目标地址仍各自对应一个 bounded candidate。本地 `Proxy/Custody`、`exchange_aggregator` 角色属于既有推断；直接 UI-TXID 链接、`Binance: Alpha Hot Wallet -> 0x73d8...46db` 映射及截图所指托管角色保持 unresolved。运行分类保持 `alpha_custody_movement_unresolved / direction=unknown / runtime_effect=none / report_only`。
...
下一步唯一需要用户执行的动作：在 Binance App 中逐条点开三行 `TXID`，提供实际跳转链接或完整 hash，用于和三个 bounded candidate 逐条比较。
